          ...
          */
            ...
            if (finishedProcess.forkId == 0)
            {
                ...
                else
                {
                    ...
                    allocateMemory(&finishedProcess);
                    fprintf(memLogPtr, "#At time %d allocated %d bytes for process %d from %d to %d\n",
                    getClk(),finishedProcess.allocatedMem->blockSize, finishedProcess.id, finishedProcess.allocatedMem->startLoc, finishedProcess.allocatedMem->endLoc);
            ...
            else
            {
                ...
                fprintf(memLogPtr, "#At time %d freed %d bytes for process %d from %d to %d\n",
                    getClk(),finishedProcess.allocatedMem->blockSize, finishedProcess.id, finishedProcess.allocatedMem->startLoc, finishedProcess.allocatedMem->endLoc);
                deallocateMemory(&finishedProcess);
...
int main(int argc, char *argv[])
{
    ...
    memLogPtr=fopen("memory.log", "w");
    if(filePtr == NULL || memLogPtr==NULL)
    {
        printf("Error in open the file\n");
    }
    printf("loogging...\n");
    fprintf(filePtr, "#At time x process y state arr w total z remain y wait k\n");
    fprintf(memLogPtr, "#At time x allocated y bytes for process z from i to j\n");
